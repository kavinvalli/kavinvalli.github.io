      ...
      <div className="breakout">
        <div className="overflow-x-auto overscroll-x-contain [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <div className="grid min-w-min grid-cols-[auto_1fr] gap-x-2 gap-y-[0.3rem]">
            <div
              aria-hidden="true"
              className="col-start-1 row-start-2 grid grid-rows-[repeat(7,var(--gh-cell))] gap-[var(--gh-gap)] text-right font-mono text-[0.65rem] leading-[var(--gh-cell)] text-faint"
            >
            ...
            {/* labels overflow their 15px column rather than stretching the grid */}
            <div
              aria-hidden="true"
              className="col-start-2 row-start-1 grid auto-cols-[var(--gh-cell)] grid-flow-col gap-[var(--gh-gap)] font-mono text-[0.65rem] leading-none text-faint [&>span]:overflow-visible [&>span]:whitespace-nowrap"
            >
              {labels.map((label, i) => (
        ...
        <div className="mt-3 flex items-center gap-1 font-mono text-[0.65rem] lowercase text-faint">
          <span className="mr-[0.15rem]">Less</span>
          {[0, 1, 2, 3, 4].map((level) => (
            <span
              key={level}
              data-level={level}
              className={`${cell} !w-[10px] !h-[10px]`}
            />
          ))}
